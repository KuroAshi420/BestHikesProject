...
export const updateLikes = (idEvent,like) =>  (dispatch) => {
  axios
   .put(`http://localhost:5000/events/like/${idEvent}`,like)
   .then((res) => dispatch(getEvents()))
   .catch((err) => console.log(err));
};
...
export const updatePlacesParticipants =  (idEvent,idUser,updatedPlaces) => async (dispatch) => {
  await axios
    .put(`http://localhost:5000/events/participantInEventUpdatePlaces/${idEvent}/${idUser}/${updatedPlaces}`)
    .then((res) => dispatch({ type: GET_PARTICIPANT, payload: res.data }))
    .catch((err) => console.log(err));
};