...
router.get("/", (req, res) => {
  Event.find()
    .populate('participant.participant')
...
//update nombre de places pour le participant
// router.put("/participantInEventUpdatePlaces/:_idEvent/:_idUser",  (req, res) =>{
//       const _id = req.params._idEvent 
//   Event.findOneAndUpdate(  {
//     _id ,
//     'participant._id':  req.params._idUser 
//   },
//   {$addToSet : {"participant" : {'_id' : req.params._idUser  , 'NombrePlace' : req.body.nombreDePlace }} }, { new: true })
//           .then((event) => res.json(event))
//           .catch((err) => res.json(err));
// })

router.put("/participantInEventUpdatePlaces/:_idEvent/:_idUser/:places",  (req, res) =>{
  const _id = req.params._idEvent 
  const _idUser = req.params._idUser 
  const places = req.params.places;
Event.findOneAndUpdate(  {
_id,
'participant.participant':  _idUser
},
 {$inc:{"participant.$.NombrePlace" : places }} , { new: true })
      .then((event) => res.json(event))
      .catch((err) => res.json(err));
})
